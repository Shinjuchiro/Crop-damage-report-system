    ...
    <x-ui.card title="Assigned Inspections" :padded="false"
        ...
        @else
            {{-- ---------- PHONE: one card per report ----------
                 ...
                 sideways. Same card treatment as
                 technician/reports/index.blade.php so the two pages behave
                 the same way.
            --}}
            <ul class="divide-y divide-border sm:hidden">
                @foreach ($reports as $report)
                    ...
                    <li>
                        <a href="{{ $cardHref }}" class="block px-4 py-4 transition active:bg-muted">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold">{{ $report->reference }}</p>
                                    <p class="truncate text-sm">{{ $report->farmer?->full_name ?? 'Unknown' }}</p>
                            ...
                            <dl class="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-muted-foreground">
                                ...
                                <div>
                                    <dt class="inline font-medium">Damage:</dt>
                                    <dd class="inline">
                                        @if ($cardShown === null)
                                            -
                                        @else
                                            {{ round($cardShown) }}% ({{ $cardAssessed !== null ? 'you assessed' : 'farmer estimate' }})
                                        @endif
                                    </dd>
                                </div>
                                @if ($report->photos_count)
                                    <div>
                                        <dt class="inline font-medium">Photos:</dt>
                                        <dd class="inline">{{ $report->photos_count }}</dd>
                                    </div>
                                @endif
                            </dl>
                            ...
                            <p class="mt-2.5 text-sm font-semibold text-primary">
