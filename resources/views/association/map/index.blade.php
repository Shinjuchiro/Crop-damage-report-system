...
{{--
    ...
    one dot). Two pin layers per section 49: where the farmer said the
    damage was, and where the technician actually verified it on the ground.
    ...
    <div class="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            ...
            <div class="inline-flex items-center gap-4 rounded-lg border border-input px-4 py-2 text-xs text-muted-foreground"
                ...
                <label class="flex cursor-pointer items-center gap-2">
                    <input type="checkbox" x-model="farmerPins"
                           @change="window.toggleAssocMapLayer && window.toggleAssocMapLayer('farmer', farmerPins)"
                           class="h-3.5 w-3.5 rounded border-input text-primary focus:ring-ring">
                    Farmer-reported
                </label>
                <label class="flex cursor-pointer items-center gap-2">
                    <input type="checkbox" x-model="verifiedPins"
                           @change="window.toggleAssocMapLayer && window.toggleAssocMapLayer('verified', verifiedPins)"
                           class="h-3.5 w-3.5 rounded border-input text-primary focus:ring-ring">
                    Technician-verified
                </label>
            </div>
        ...
        <div class="border-t border-border px-6 py-4">
            ...
            <p class="mt-3 text-xs text-muted-foreground">
                Verified pins are coloured by the assessed severity. This map only shows {{ $association->name }}'s own
                members - for the office-wide picture across all associations, ask the Municipal Agriculture Office.
            </p>
        </div>
    </div>
</div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.toggleAssocMapLayer = function (which, on) {
