...
{{--
    Reset Password. Reached only from the emailed link (auth.forgot-password
    -> PasswordResetController -> this page, token + email in the URL).
    Same mobile/desktop split shell as login and forgot-password - see
    auth.login's comment for why the two layouts are kept fully separate
    rather than shared.
--}}
...
<div class="lg:hidden">
    ...
    <div class="relative -mt-10 min-h-[60vh] rounded-t-3xl bg-card px-5 pb-10 pt-7">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div class="min-w-0">
                <h2 class="text-2xl font-bold text-[#0d3d1b] dark:text-foreground">Set a New Password</h2>
                <p class="text-sm text-muted-foreground">Choose something you'll remember</p>
            </div>
        </div>
...
<div class="hidden min-h-screen lg:grid lg:grid-cols-2">
    ...
    <div class="flex items-center justify-center bg-card px-12 py-12">
        <div class="w-full max-w-sm">
            ...
            <h2 class="text-3xl font-bold text-[#0d3d1b] dark:text-foreground">Set a New Password</h2>
            <p class="mt-1 text-sm text-muted-foreground">Choose something you'll remember.</p>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="mt-5 space-y-4">
                ...
                <div class="space-y-1.5">
                    <label for="email-reset-desktop" class="block text-sm font-medium">Email address</label>
                    <input id="email-reset-desktop" type="email" name="email" value="{{ old('email', $email) }}"
                ...
                <div class="space-y-1.5">
                    <label for="password-reset-desktop" class="block text-sm font-medium">New password</label>
                    <input id="password-reset-desktop" type="password" name="password" required
                ...
                <div class="space-y-1.5">
                    <label for="password-confirmation-desktop" class="block text-sm font-medium">Confirm new password</label>
                    <input id="password-confirmation-desktop" type="password" name="password_confirmation" required
                           autocomplete="new-password"
                           class="h-11 w-full rounded-lg border border-input bg-card px-3.5 text-sm
                                  focus:border-primary">
                </div>
