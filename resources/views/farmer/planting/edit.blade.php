...
@section('title', 'Edit Crop Planting')
@section('heading', 'Edit Crop Planting')
@section('subheading', 'Fix a mistake in what you already submitted. Ayusin ang pagkakamali sa inyong nai-submit na.')

@section('content')

{{--
    Same shape as the create form (sections 24-27), just prefilled and
    posting to update() instead of store(). Farmers can correct their own
    mistakes here, but - unlike MAO's monitoring page - cannot archive a
    record; only the office retires one from the active list.
--}}
...
<div
    ...
    <form method="POST" action="{{ route('farmer.planting.update', $record) }}"
        ...
        <x-ui.card title="Crops planted" description="Add one block for every crop you planted.">
            ...
            <x-slot:footer>
                <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    <x-ui.button size="lg" variant="outline" :href="route('farmer.planting.show', $record)">
                        Cancel
                    </x-ui.button>
                    <x-ui.button size="lg" type="submit">Review &amp; Save</x-ui.button>
                </div>
            </x-slot:footer>
        </x-ui.card>
    </form>
...
<script>
    function plantingForm(crops, initialRows) {
        return {
            ...
            rows: (initialRows && initialRows.length) ? initialRows : [emptyRow(), emptyRow()],
